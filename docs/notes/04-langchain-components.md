...
![HLD-Index](../images/04-indexes-pipeline.drawio.svg)
...
In code, the two calls are simply two independent `invoke`s:
...
print(model.invoke("Who is Narendra Modi?").content)
# → "Narendra Modi is an Indian politician ... Prime Minister of India."

print(model.invoke("How old is he?").content)
# → "As an AI, I don't have access to personal data about individuals ..."
```

Same model object, same API key, seconds apart — and the second request carries none of the first.

Diagram: 

![stateless-llm-calls](../images/04-stateless-llm-calls.drawio.svg).

The second call cannot decode "he", because it never saw the first call. **And this is a big problem** — a chatbot built on a stateless system is frustrating to talk to; you would have to remind it of the context in every single message.
...
The component that makes building AI agents easy — and the topic everyone has been talking about for the last six months ("AI agents are the next big thing").
...
![`agent-reasoning-loop`](../images/04-agent-reasoning-loop.drawio.svg)
